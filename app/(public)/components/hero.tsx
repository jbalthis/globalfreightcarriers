...
const Hero = () => {
  ...
  const text_gradient = 'bg-gradient-to-br from-blue-200 via-lime-300 to-indigo-800 text-transparent bg-clip-text'

  return (
    <section id="home" className={`flex md:flex-row flex-col ${styles.paddingY}`}>
      <div className={`flex-1 ${styles.flexStart} flex-col xl:px-0 sm:px-16 px-6`}>
        <div className={`flex flex-row items-center py-4 px-4 ${bg_discount_gradient} rounded-[10px] mb-2`}>
         <AreaChart className="w-[24px] h-[24px] text-white" />
          <p className={`${styles.paragraph} ml-2`}>
            <span className="text-white">Over <span className="italic text-lime-300">20 years</span> Freight Shipping Experience with the</span><br />
            <span className="text-white"><span className="italic text-lime-300">Lowest</span> Freight Hauling<span className="italic text-lime-300">Rates</span> in North America</span>
          </p>
        </div>
        ...
        <p className={`${styles.paragraph} max-w-[470px] mt-5 text-gray-800`}>
            Global Freight Carriers provides simple, affordable solutions for all your shipping needs, locally or globally. Whether it's oversize, expedited, refrigerated, or super loads, one call to us handles it all!
        </p>
      </div>
